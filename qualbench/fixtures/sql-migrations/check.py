...
def run_task(task: dict, manifest: dict, base_url: str, model: str, timeout: int,
    ...
    if fixed_sql is None:
        return False, ["could not extract SQL from model response"], time.time() - t0
    ...
    try:
        ...
        return False, [f"verify.sh failed (exit {result.returncode}):", tail], elapsed
    except subprocess.TimeoutExpired:
        return False, ["verify.sh timed out"], time.time() - t0
    finally:
